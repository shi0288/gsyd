...
public class ArcherMessages extends OnloadScript {
	...
	private static final String MESSAGE_OPEN_URL = "{'clientId':'','message':{'severity':'%s','summary':'%s','detail':'%s'},'yesValue':'%s','yesUrl':'%s','isBlank':%s}";
	...
	public void encodeChildren(final FacesContext context) throws IOException {
		...
		if (form != null) {
			if (context.isValidationFailed()) {
				...
				form.visitTree(VisitContext.createVisitContext(context, null,
						VISIT_HINTS), new VisitCallback() {
							...
							if (msgs.size() >= 1) {
								try {
									...
									jo = new JSONObject(String.format(
											MESSAGE,
											clientId,
											msgs.get(0).getSeverity()
													.getOrdinal(),
											msgs.get(0).getSummary()
													.replace("\'", "\\\'"),
											msgs.get(0).getDetail()
													.replace("\'", "\\\'")));
									messages.put(jo);
		...
		while (msgsWithoutId.hasNext()) {
			...
			try {
				if (facesMessage instanceof OpenUrlFacesMessage) {
					OpenUrlFacesMessage ouMessage = (OpenUrlFacesMessage) facesMessage;
					messages.put(new JSONObject(String.format(MESSAGE_OPEN_URL,
							ouMessage.getSeverity().getOrdinal(),
							ouMessage.getSummary(), ouMessage.getDetail(),
							ouMessage.getYesValue(), ouMessage.getYesUrl(),
							ouMessage.isBlank())));
				} else {
				...
				throw new RuntimeException("jsonString: "
						+ String.format(MESSAGE, "", facesMessage.getSeverity()
								.getOrdinal(), facesMessage.getSummary(),
			...
			context.getResponseWriter().write(
					String.format(SCRIPT,
							messages.toString().replace("\\\"", "\\\\\"")
									.replace("\'", "\\\'"), getStyleClass(),
							isFocus(), isShowGlobalMessages()));
		}
	}
