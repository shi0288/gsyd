...
public class InvestStatistics {
	 ...
	 * @return
	 */
	public double getAllReceivedCorpusInterest() {
		String hql = "Select sum(corpus)+sum(interest+defaultInterest-fee) from InvestRepay where time is not null and time <= ?";
		List result = this.ht.find(hql, new Date());

		double money = 0.0D;
		if ((result != null) && (result.get(0) != null)) {
			money = ((Double) result.get(0)).doubleValue();
		}
	 ...
	 * @return
	 */
	public double getAllReceivableCorpusInterest() {
		String hql = "Select sum(corpus)+sum(interest+defaultInterest-fee) from InvestRepay where time is null and time <= ?";
		List result = this.ht.find(hql, new Date());
