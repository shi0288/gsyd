...
public class TransferStatistics {
	...
	public long getCountByUserOut(String userId) {
		String hql = "select count(i) from TransferApply i "
				+ "where i.invest.user.id =? and i.status=?";
		List<Object> oos = ht.find(hql, userId, TransferStatus.TRANSFED);
		Object o = oos.get(0);
		if (o == null) {
			return 0;
		}
	...
	public long getCountByUserIn(String userId) {
		String hql = "select count(i) from Invest i "
				+ "where i.user.id=? and i.transferApply is not null and i.status=?";
		List<Object> oos = ht.find(hql, userId, InvestStatus.REPAYING);
		Object o = oos.get(0);
		if (o == null) {
			return 0;
		}
