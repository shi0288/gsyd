...
public class BorrowerAuthenticationHome extends EntityHome<BorrowerAuthentication> {
	...
	@Override
	public String save() {
		if(null==getInstance().getIdCardPhoto()||null==getInstance().getIdCardPhoto().getId()){
				FacesUtil.addErrorMessage("手持身份证照片,未上传!");
				return null;
		}
		if(null==getInstance().getBankCreditReport()||null==getInstance().getBankCreditReport().getId()){
				FacesUtil.addErrorMessage("银行征信报告,未上传!");
				return null;
		}
		if(null==getInstance().getDiplomaScan()||null==getInstance().getDiplomaScan().getId()){
				FacesUtil.addErrorMessage("学历证书扫描件,未上传!");
				return null;
		}
		if(null==getInstance().getProofEarnings()||null==getInstance().getProofEarnings().getId()){
				FacesUtil.addErrorMessage("收入证明,未上传!");
				return null;
		}
