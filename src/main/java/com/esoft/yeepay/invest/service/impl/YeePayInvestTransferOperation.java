...
public class YeePayInvestTransferOperation extends
	...
	public TrusteeshipOperation createOperation(Invest invest, FacesContext fc)
		...
		fd.setAmount(String.valueOf(buyPrice));
			...
			if("1".equals(code)){//易宝冻结金额成功
				...
				if(result){//易宝转账确认成功
					...
					if(invest != null && ta != null){
						...
						try {
							userBillBO.transferOutFromFrozen(invest.getUser().getId(),buyPrice,OperatorInfo.TRANSFER, "债权：" + invest.getId()+ "购买成功");
							userBillBO.transferIntoBalance(ta.getInvest().getUser().getId(),buyPrice,OperatorInfo.TRANSFER, "债权：" + invest.getId() + "转让成功");
						} catch (InsufficientBalance e) {
							log.debug("债权转让平台划款时出错！");
					...
					if(invest != null && ta != null){
						...
						try {
							...
							log.debug("购买债权："+ invest.getId() + "失败，解冻金额时出错！");
	...
	public void receiveOperationS2SCallback(ServletRequest request,
		...
		if(flag){//验签成功
			...
			throw new YeePayOperationException("债权转让验签失败");
		}
