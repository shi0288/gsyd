...
//            }
    ...
    public RechargeBatchUpload(){
    }
    
    public StreamedContent getTemplate() {  
    	InputStream stream = ((ServletContext)FacesContext.getCurrentInstance().getExternalContext().getContext()).getResourceAsStream("/upload/批量充值模板.xls"); 
    	String fileName;
    	try {
			fileName = new String(("批量充值模板.xls").getBytes("utf-8"), 	"iso8859-1");
		} catch (UnsupportedEncodingException e) {
			fileName = "template.xls";
		}
    	template = new DefaultStreamedContent(stream, "application/vnd.ms-excel", fileName);  
        return template;  
    }
}
