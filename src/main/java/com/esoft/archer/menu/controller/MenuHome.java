...
//		menu.setPermission(new Permission());
		...
		if(StringUtils.isNotEmpty(parentId)){
			Menu p = getBaseService().get(Menu.class, parentId);
			menu.setParent(p);
		}else{
	...
	public String save() {
//		// 当前菜单的上级菜单不能是它本身
		if (getInstance().getParent() != null && parentIsItself(getInstance().getId(), getInstance().getParent().getId())) {
			return FacesUtil.redirect("/admin/menu/menuEdit?id="
					+ getInstance().getId() + "&typeId="
					+ getInstance().getMenuType().getId());
		}
		// 父菜单不能是它的子菜单
		if (getInstance().getParent() != null && parentIsChild(getInstance().getId(), getInstance().getParent()
				.getId())) {
			return FacesUtil.redirect("/admin/menu/menuEdit?id="
					+ getInstance().getId() + "&typeId="
					+ getInstance().getMenuType().getId());
...
//		}
	...
	public String delete() {
		...
		setInstance(getBaseService().get(Menu.class, getId()));
