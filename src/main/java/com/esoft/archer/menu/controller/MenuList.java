...
public class MenuList extends EntityQuery<Menu> implements java.io.Serializable {
	...
	public String getTempMenuTypeId() {
		return tempMenuTypeId;
	}

	public void setTempMenuTypeId(String tempMenuTypeId) {
		this.tempMenuTypeId = tempMenuTypeId;
	}
	...
	public String getParentIdByChildUrl(String url) {
		...
		if (parentId == null && StringUtils.equals("node", params[1])
				&& !url.contains("%")) {
	...
	public TreeNode getRoot() {
		...
		if (menuList != null && menuList.size() > 0) {
			List<Menu> mwnp = filtrateMenuWithNullParent(menuList);
			for (Menu m : mwnp) {
				TreeNode newChild = createNewNode(m, root);
				initTreeNode(menuList, m, newChild);
			}
			// 把有父节点，但是不在查询结果中的menu
			for (Menu m : menuList) {
				TreeNode newChild = createNewNode(m, root);
			}
		}
	...
	private void initTreeNode(List<Menu> menuList, Menu parentMenu,
			TreeNode parentNode) {
		List<Menu> mcs = filtrateChildren(menuList, parentMenu);
		for (Menu menu : mcs) {
	...
	/**
	 * 筛选出所有的父节点，只要符合条件的，就在列表中删除掉，提高性能。
	 * 
	 * @param menus
	 * @return
	 */
	private List<Menu> filtrateMenuWithNullParent(List<Menu> menus) {
	...
	/**
	 * 筛选出所有的孩子节点，只要符合条件的，就在列表中删除掉，提高性能
	 * 
	 * @param menus
	 * @param parent
	 * @return
	 */
	private List<Menu> filtrateChildren(List<Menu> menus, Menu parent) {
		List<Menu> rs = new ArrayList<Menu>();
		for (Iterator iterator = menus.iterator(); iterator.hasNext();) {
			Menu menu = (Menu) iterator.next();
			if (menu.getParent() != null
	...
	public List<Menu> getMenuTree() {
		//页面父菜单更新  update by w44
		if(StringUtils.isNotEmpty(tempMenuTypeId)){
			if(!tempMenuTypeId.equals(menuTypeId)){
				menuTree=null;
			}
		}
		//end
		if (menuTree == null) {
			...
			List<Menu> list = ht.findByNamedQuery("Menu.findMenusByTypeId",
					menuTypeId);
			this.setTempMenuTypeId(menuTypeId);
	...
	private void buildMenuTree(Menu parent, List<Menu> menuList) {
		for (int i = 0; i < menuList.size(); i++) {
			...
			if (menu.getParent() != null
				...
				menu.setLabel(prefixStr + menu.getLabel());
				menuTree.add(menu);
