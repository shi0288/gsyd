...
//        this.buttonBar.removeClass('ui-widget-header');
...
//                        row.children('td.ui-fileupload-progress').append('<div class="ui-progressbar ui-widget ui-widget-content ui-corner-all" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="ui-progressbar-value ui-widget-header ui-corner-left" style="display: none; width: 0%;"></div></div>');
            ...
            send: function(e, data) {
            	$this.chooseButton.children('input').attr('disabled','disabled');
            	$this.chooseButton.attr('aria-disabled',true);
            	$this.chooseButton.addClass("ui-state-disabled");
//            	$this.disableButton($this.chooseButton);
//                if(!window.FormData) {
...
//                    }
                    if (typeof(growl) != "undefined") {
                    	growl.removeAll();
                    	growl.renderMessage({summary:'正在上传', detail:progress + '%', severity: 'info'});
            		} else if(typeof($.dialog)!="undefined"){
            			$.dialog.tips("正在上传"+progress + '%', 2, false, 'info-primefaces');
            		}
                }
            },
            done: function(e, data) {
            	$this.chooseButton.children('input').attr('disabled',false);
            	$this.chooseButton.attr('aria-disabled',false);
            	$this.chooseButton.removeClass("ui-state-disabled");
            	
//            	$this.enableButton($this.chooseButton);
...
//        this.messageContainer.show();
        if (typeof(growl) != "undefined") {
        	growl.renderMessage({summary:summary, detail:detail, severity: 'error'});			
		} else if(typeof($.dialog)!="undefined"){
			$.dialog.tips(summary, 2, true, 'error-primefaces');
		}
    },
